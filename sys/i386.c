...
setsegs()
{
    ...
    // What we have is:
    //
    //      /* 8 backing stores - Pages 2048 - 4095 (8MB) */
    //      /* Free frames      - Pages 1024 - 2047 (4MB) */
    //      /* Kernel/System    - Pages    0 - 1023 (4MB) */
    //
    // Therefore maxaddr for kernel/system mem is (1024 * NBPG - 1)
    //
    maxaddr = (char *)( 1024 * NBPG - 1); /* 4M size */


    // XXX
    // Set the DS and SS segments' limits to their highest values. This
    // will allow processes to use memory up to the 4 GB limit without
    ...
    //
    // Determine the segment information for the kernel code segment.
    // The end of the segment can be gathered from etext (this is the
    // first address past the end of the text segment, also known as the
    // the program code). We will determine the number of pages in
    // this section and then set the limit for memory protection
    // purposes. The limit (int) is broken up into hilimit (short) and
    // lolimit (short) variables. 
    //
    ...
    */
    ...
    // Global kernel memory (getmem(), getstk()) stops at page 1024 (4M). 
    //
    // initsp = 4M - 4bytes.. will grow down
    initsp = 1024*NBPG  - 4;
}
